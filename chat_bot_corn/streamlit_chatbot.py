...
st.set_page_config(page_title="ChatBot-dev Pro", page_icon="👾")
st.markdown("<h1 style='text-align: center; font-size: 32px;'>ChatBot-dev Pro👾</h1>", unsafe_allow_html=True)
...
prompt_template = PromptTemplate(
   ...
   template="""Eres un asistente útil y amigable llamado ChatBot-dev Pro.
...
if pregunta:
   # Mostrar mensaje del usuario en la interfaz
    ...
    # Almacena de mensaje en memoria de streamlist
    st.session_state.mensajes.append({
       "mensajes" : HumanMessage(content=pregunta),
       "hora" : obtener_hora()
    })

    # Genera respuesta usando el modelo del lenguaje
    respuesta = chat_model.invoke([item["mensajes"] for item in st.session_state.mensajes])
